    ...
    var body: some View {
        ...
        Canvas { graphicsContext, size in
            ...
            for mark in marks {
                ...
                if mark.outer {
                    points = outerRing.arcPoints(lambdas: changePhase(phase: startingAngle, angles: mark.locations.filter { $0 >= 0 && $0 < 1 }))
                } else {
                    points = innerRing.arcPoints(lambdas: changePhase(phase: startingAngle, angles: mark.locations.filter { $0 >= 0 && $0 < 1 }))
                }
