        ...
        func interpolate(at: CGFloat) -> CGColor {
            ...
            if let nextIndex = nextIndex {
                ...
                if previousIndex >= locations.startIndex {
                    let leftColorComponents = colors[previousIndex].components!
                    let rightColorComponents = colors[nextIndex].components!
                    let ratio = (at - locations[previousIndex]) / (locations[nextIndex] - locations[previousIndex])
                    guard ratio <= 1 && ratio >= 0 else { fatalError() }
                    let components = [leftColorComponents[0] * (1-ratio) + rightColorComponents[0] * ratio,
                                      leftColorComponents[1] * (1-ratio) + rightColorComponents[1] * ratio,
                                      leftColorComponents[2] * (1-ratio) + rightColorComponents[2] * ratio,
                                      leftColorComponents[3] * (1-ratio) + rightColorComponents[3] * ratio]
                    let newColor = CGColor(colorSpace: colors[previousIndex].colorSpace ?? displayP3, components: components)
                    return newColor ?? colors[previousIndex]
                } else {
                    return colors.first!
                }
            } else {
