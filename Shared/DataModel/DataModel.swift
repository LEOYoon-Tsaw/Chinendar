...
extension DataSchema {
    ...
    static let container = {
        ...
        let modelConfig = ModelConfiguration("ChineseTime", schema: fullSchema, url: containerPath, cloudKitDatabase: .automatic)
...
extension ThemeData {
    ...
    static func latestVersion() -> Int {
        ...
        let version = try? DataSchema.context.fetch(descriptor).first?.version
        return version ?? 0
    }
...
extension LocalSchema {
    ...
    static let container = {
        ...
        let modelConfig = ModelConfiguration("ChineseTimeLocal", schema: localSchema, groupContainer: .automatic, cloudKitDatabase: .none)
